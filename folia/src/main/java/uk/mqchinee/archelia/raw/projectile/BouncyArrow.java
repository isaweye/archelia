    ...
    private void bounce(Projectile projectile) {
        ...
        if (blockFace != null) {
            ...
            if (getVelocity().length() > threshold) {
                Vector finalDir = dir;
                Bukkit.getRegionScheduler().run(Archelia.getInstance(), projectile.getLocation(), (task) -> {
                    Projectile newProjectile = (Projectile) projectile.getWorld().spawnEntity(projectile.getLocation(), projectile.getType());
                    newProjectile.setVelocity(projectile.getVelocity().subtract(finalDir).normalize().multiply(getVelocity().length()));
