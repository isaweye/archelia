package uk.mqchinee.archelia.velocity;
...
public class VelocityManager {
    ...
    public void dash(Entity e, int x, int y, int z) {
        ...
        if (y > 60) {y = 60;}
        if (z > 43) {z = 43;}
        e.setVelocity(new Vector(VelocityReference.getXZVelocities(x)*getNeg(xNeg), VelocityReference.getYVelocities().get(y)*getNeg(yNeg),
                VelocityReference.getXZVelocities(z)*getNeg(zNeg)));
    }
    ...
    public void dash(Entity e, int x, int y, Double z) {
        ...
        if (x > 43) {x = 43;}
        if (y > 60) {y = 60;}
        e.setVelocity(new Vector(VelocityReference.getXZVelocities(x)*getNeg(xNeg), VelocityReference.getYVelocities().get(y)*getNeg(yNeg), z));
    }
    ...
    public void dash(Entity e, int x, Double y, Double z) {
        ...
        if (x < 0) {x = Math.abs(x);xNeg = true;}
        if (x > 43) {x = 43;}
        e.setVelocity(new Vector(VelocityReference.getXZVelocities(x)*getNeg(xNeg), y, z));
    }
    ...
    public void dash(Entity e, Double x, int y, Double z) {
        ...
        if (y < 0) {y = Math.abs(y);yNeg = true;}
        if (y > 60) {y = 60;}
        e.setVelocity(new Vector(x, VelocityReference.getYVelocities().get(y)*getNeg(yNeg), z));
    }
    ...
    public void dash(Entity e, Double x, int y, int z) {
        ...
        if (y > 60) {y = 60;}
        if (z > 43) {z = 43;}
        e.setVelocity(new Vector(x, VelocityReference.getYVelocities().get(y)*getNeg(yNeg), VelocityReference.getXZVelocities(z)*getNeg(zNeg)));
    }
    ...
    public void dash(Entity e, Double x, Double y, int z) {
        ...
        if (z < 0) {z = Math.abs(z);zNeg = true;}
        if (z > 43) {z = 43;}
        e.setVelocity(new Vector(x, y, VelocityReference.getXZVelocities(z)*getNeg(zNeg)));
    }
    ...
    public void dash(Entity e, int x, Double y, int z) {
        ...
        if (x > 43) {x = 43;}
        if (z > 43) {z = 43;}
        e.setVelocity(new Vector(VelocityReference.getXZVelocities(x)*getNeg(xNeg), y, VelocityReference.getXZVelocities(z)*getNeg(zNeg)));
    }
    ...
    public void dash(Entity e, int x, int y, int z, boolean additive, boolean adjustHorizontal, boolean adjustVertical) {
        ...
        if (adjustHorizontal) {
            ...
            double dashAngle = 90 + Math.atan2(VelocityReference.getXZVelocities(z)*getNeg(zNeg), VelocityReference.getXZVelocities(x)*getNeg(xNeg))
                    * convertDegrees;
            double velocityLength = Math.sqrt(Math.pow(VelocityReference.getXZVelocities(z)*getNeg(zNeg), 2) +
                    Math.pow(VelocityReference.getXZVelocities(x)*getNeg(xNeg), 2));
            ...
            newZ = VelocityReference.getXZVelocities(z)*getNeg(zNeg);
            newX = VelocityReference.getXZVelocities(x)*getNeg(xNeg);
        }
        if (additive) {
            e.setVelocity(new Vector(newX+e.getVelocity().getX(), VelocityReference.getYVelocities().get(y)*getNeg(yNeg)*getVert(adjustVertical,
                    e.getLocation().getDirection())+e.getVelocity().getY(), newZ+e.getVelocity().getZ()));
        } else {e.setVelocity(new Vector(newX, VelocityReference.getYVelocities().get(y)*getNeg(yNeg)*getVert(adjustVertical, e.getLocation().getDirection()),
                newZ));}
    }
    ...
    public void dash(Entity e, int x, int y, Double z, boolean additive, boolean adjustHorizontal, boolean adjustVertical) {
        ...
        if (adjustHorizontal) {
            ...
            double entityLook = -Math.atan2(entityDir.getZ(), entityDir.getX()) * convertDegrees + 90;
            if (entityLook < 0) {entityLook += 360;}
            double dashAngle = 90 + Math.atan2(z, VelocityReference.getXZVelocities(x)*getNeg(xNeg)) * convertDegrees;
            double velocityLength = Math.sqrt(Math.pow(z, 2) + Math.pow(VelocityReference.getXZVelocities(x)*getNeg(xNeg), 2));
            double newDashAngle = entityLook - dashAngle + 90;
            if (newDashAngle < 0) {newDashAngle += 360;}
            z = Math.cos(newDashAngle / convertDegrees) * velocityLength;
            newX = Math.sin(newDashAngle / convertDegrees) * velocityLength;
        } else {newX = VelocityReference.getXZVelocities(x)*getNeg(xNeg);}
        if (additive) {
            e.setVelocity(new Vector(newX+e.getVelocity().getX(), VelocityReference.getYVelocities().get(y)*getNeg(yNeg)*getVert(adjustVertical,
    ...
    public void dash(Entity e, int x, Double y, Double z, boolean additive, boolean adjustHorizontal, boolean adjustVertical) {
        ...
        if (adjustHorizontal) {
            ...
            double entityLook = -Math.atan2(entityDir.getZ(), entityDir.getX()) * convertDegrees + 90;
            if (entityLook < 0) {entityLook += 360;}
            double dashAngle = 90 + Math.atan2(z, VelocityReference.getXZVelocities(x)*getNeg(xNeg))
                    * convertDegrees;
    ...
    public void dash(Entity e, Double x, int y, Double z, boolean additive, boolean adjustHorizontal, boolean adjustVertical) {
        ...
        if (additive) {
            e.setVelocity(new Vector(x+e.getVelocity().getX(), VelocityReference.getYVelocities().get(y)*getNeg(yNeg)*getVert(adjustVertical,
                    e.getLocation().getDirection())+e.getVelocity().getY(), z+e.getVelocity().getZ()));
        } else {e.setVelocity(new Vector(x, VelocityReference.getYVelocities().get(y)*getNeg(yNeg)*getVert(adjustVertical, e.getLocation().getDirection()),
                z));}
    }
    ...
    public void dash(Entity e, Double x, int y, int z, boolean additive, boolean adjustHorizontal, boolean adjustVertical) {
        ...
        if (adjustHorizontal) {
            ...
            double entityLook = -Math.atan2(entityDir.getZ(), entityDir.getX()) * convertDegrees + 90;
            if (entityLook < 0) {entityLook += 360;}
            double dashAngle = 90 + Math.atan2(VelocityReference.getXZVelocities(z)*getNeg(zNeg), x) * convertDegrees;
            double velocityLength = Math.sqrt(Math.pow(VelocityReference.getXZVelocities(z)*getNeg(zNeg), 2) + Math.pow(x, 2));
            double newDashAngle = entityLook - dashAngle + 90;
            if (newDashAngle < 0) {newDashAngle += 360;}
            newZ = Math.cos(newDashAngle / convertDegrees) * velocityLength;
            x = Math.sin(newDashAngle / convertDegrees) * velocityLength;
        } else {newZ = VelocityReference.getXZVelocities(z)*getNeg(zNeg);}
        if (additive) {
            e.setVelocity(new Vector(x+e.getVelocity().getX(), VelocityReference.getYVelocities().get(y)*getNeg(yNeg)*getVert(adjustVertical,
                    e.getLocation().getDirection())+e.getVelocity().getY(), newZ+e.getVelocity().getZ()));
        } else {e.setVelocity(new Vector(x, VelocityReference.getYVelocities().get(y)*getNeg(yNeg)*getVert(adjustVertical, e.getLocation().getDirection()),
